...
> *(Added **Critical Theory & the Frankfurt School** as `v9-critical-theory-and-the-frankfurt-school` — filed under the existing **v9 / Group A — Disciplines & Fields** cohort as `id_code` **A7**, discipline **5 Culture, History & Worldview**, `likely_phase` 5, rather than minting a `v10` tag for one row (a new version would add a whole colour/badge/filter facet for a single topic). Followed the v9 Group A field dialect: `tier`/`phase`/`phase_rationale`/`primary_source` left empty (parser falls back to `group_code` for tier). 7 key figures (Adorno, Horkheimer, Benjamin, Marcuse, Habermas, Gramsci, Fraser) and 10 connections — **all 10 resolved**, no faded chips. Regenerated `data.js` → 204 topics / 844 edges. Hit a CSV gotcha: the file has a **CRLF header** but LF-terminated records, so a naive header split produced `source_date\r` and would have misaligned the row — appended LF to match the surrounding records. Verified in-browser (Catalog card, detail panel, 10 graph edges, all 7 figures in Thinkers, deep-link hash) with no console errors; tests 20/20 incl. the zero-warnings integration test. Docs: README counts, ARCHITECTURE thinker count, new LEARNINGS.md, PROMPT, memory.)*

> Reciprocal links should always be updated when required.
> *(Established as a standing curation rule. Added the back-link to Critical Theory on all **10** target rows — Media Theory, Foucault, Political Philosophy, Semiotics, Aesthetics, Attention Economy, Sociology, ANT/STS, History & Philosophy of Science, Epistemology — so the panel now reads `in 10 · out 10` with a populated *Incoming — referenced by (10)* list. Edited surgically (locate each row's exact serialized `connects_to_raw,connects_to_ids` pair, assert a single match, swap) rather than re-serializing the CSV, so the diff is 10 changed lines and nothing else; verified against `git show HEAD` that **only** the two connection columns moved and every pre-existing slot pairing survived. Regenerated `data.js` → 854 edges (+10), zero warnings, tests 20/20. Context surfaced: the dataset was only **19% reciprocated** overall (160/844) before this — the rule is now documented in README, ARCHITECTURE, and LEARNINGS, but the ~674 pre-existing one-way edges were left alone as a separate cleanup.)*

> Okay, work on the one way edges.
> *(Took the **symmetric closure** of the connection graph: **666 back-links added across 115 of 204 rows**, 854 → 1520 directed edges, **0 one-way edges** remaining. Asked two clarifying questions first (full vs selective vs runtime symmetry; and what to do with the UI built on direction) — both answered with the recommended option. Because connections are now undirected, simplified the three surfaces that assumed direction: the detail panel's Outgoing/Incoming split collapsed into one deduped **Connects to (N)** list (also drops an O(n²) scan per render), the `in N · out N` badge became `N connections`, and the Hubs view's identical In-degree/Out-degree tabs merged into **Connections** (Bridge score kept). Added a boot-time `NEIGHBOURS`/`DEGREE` map — distinct neighbours per topic — because `inDegree + outDegree` now double-counts *and* 12 rows name one topic through two raw labels ("AI"/"machine learning"); every view that ranks or sizes by connectedness reads it. `renderGraph` now collapses EDGES to one line per unordered pair: without it every relationship draws twice and `forceLink` pulls twice, contracting the deliberately loose layout — 1520 edges → **754** lines, actually *fewer* elements than the 854 before. Verified all 5 views, version-filter interaction, and slot integrity vs HEAD (only the two connection columns changed, no slot shifts); tests 20/20, zero parser warnings, no console errors. Also corrected README's "roughly 30% unresolved" to the true ~15%, which my change had made materially wrong.)*

> Okay, add the first test.
> *(Added **"integration: every connection in the real CSV is mutual"** to `parse.test.mjs` — 21 tests, all green. It folds `payload.edges` into per-topic neighbour **Sets** (so the 12 duplicate-raw-label rows collapse naturally) and asserts each edge has its reverse. Lives in the integration section, not the synthetic-CSV section, because several synthetic fixtures are deliberately one-way; and it names offenders rather than just counting, since the fix is always "append to the target row's two aligned columns". **Verified the failure path, not just the pass**: deliberately deleted one back-link (fails, naming the exact pair), then all 10 (reports 10), then all 34 to a hub (reports "…and 24 more" — the truncation branch). Restored the CSV byte-identically from a backup each time, and reverted a timestamp-only `data.js` regeneration so the commit is just the test. Docs: ARCHITECTURE and README now note that the parser cannot catch a one-way row — it is valid CSV producing a warning-free data.js — so the test is the only guard; LEARNINGS' manual check snippet replaced by a pointer to it.)*
