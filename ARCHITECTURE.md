...
- **Marked as worked on** — a binary toggle writes ids into
  ...
  without a migration.
- **Resizable rails** — the Catalog and Reader left rails are CSS-grid columns
  driven by the `--cat-rail-w` / `--reader-rail-w` variables, with a 6px
  `.rail-resizer` grid track between rail and content. One shared helper,
  `makeRailResizer(handle, container, railEl, cssVar, storageKey)`, restores the
  saved width up front, writes the dragged px into the CSS var (clamped to
  170…min(520, container−360)), and persists on release to
  `localStorage["rte:catRailW"]` / `["rte:readerRailW"]`. The drag guard keys off
  a `.dragging` class (not pointer capture). The Reader chrome is rebuilt every
  render, so its resizer is re-wired each `renderReader()`; the helper re-applies
